...
import { auth } from '../firebase/firebase'; // Assumes this is where 'auth' is exported

export default function Login() {
  const navigate = useNavigate();
  const [userId, setUserId] = useState(''); // Use email for Firebase login
  ...
  const handleSubmit = async (e: React.FormEvent) => { 
    ...
    // 🛑 CRITICAL CHANGE: Use Firebase Authentication
    try {
      await signInWithEmailAndPassword(auth, userId, password);
      // If successful, AuthProvider takes over, and we navigate
      navigate('/dashboard'); 
    } catch (err: any) {
      console.error("Firebase Login Error:", err);
      // Display a user-friendly error
      setError("Login failed. Check your User ID (Email) and Password.");
    }
  };
            ...
            <div className="absolute inset-0 rounded-[20%] border border-parksense-border bg-white flex flex-col items-center justify-center px-8 md:px-20 py-8 md:py-12">
              ...
              <div className="w-full max-w-[366px] space-y-8 md:space-y-12 lg:space-y-16">
                {error && ( // Display error if login fails
                  <p className="text-red-500 text-center font-fustat font-bold">{error}</p>
                )}
